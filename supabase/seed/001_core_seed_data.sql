...
-- Run this AFTER 20260408000000_seed_schema_alignment.sql
BEGIN;
...
INSERT INTO tenant_settings (organization_id, autonomy_mode_default, audit_retention_years, oee_enabled, governance_enabled, risk_matrix, branding)
SELECT id, 'conditional', 7, true, true,
'{"probability": ["very_low", "low", "medium", "high", "very_high"], "consequence": ["very_low", "low", "medium", "high", "very_high"]}', '{}'
FROM organizations WHERE slug = 'axium' LIMIT 1
ON CONFLICT (organization_id) DO NOTHING;

COMMIT;
SELECT 'Core seed data applied successfully' AS status;
